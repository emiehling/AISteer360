...
class TopKHeadSelector(BaseSelector[list[tuple[int, int]]]):
    ...
    def select(self, *, probe_accuracies: dict[tuple[int, int], float]) -> list[tuple[int, int]]:
        """Return the top-K (layer_id, head_id) pairs sorted by accuracy descending.

        Args:
            probe_accuracies: Mapping from (layer_id, head_id) to probe accuracy.

        Returns:
            List of top-K (layer_id, head_id) tuples, sorted by accuracy descending.
        """
        ranked = sorted(probe_accuracies.items(), key=lambda item: item[1], reverse=True)

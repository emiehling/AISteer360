...
class SinglePairFit:
    ...
    Produces `[T, H]` directions per layer (the ActAdd extraction). Fitting requires a live
    model, since the pair is co-padded with a real space token whose masked activations feed
    the positional diff, which remote capture cannot reproduce. The fitted master is memoized
    per model; every `resolve` returns an independent clone.

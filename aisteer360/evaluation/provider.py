...
from aisteer360.algorithms.core.output import Output, truncate_at_stop_strings
...
class SteeringPipelineModelAPI(ModelAPI):
    ...
    Args:
        model_name: Bare model name; Inspect renders the model as `aisteer/<model_name>`.
    ...
    def __init__(
        ...
        **model_args: Any,
        ...
        if pipeline is None:
            raise ValueError(
                "The 'aisteer' provider wraps an in-process SteeringPipeline and cannot be built from "
                "a model name. Construct it through aisteer360.evaluation.provider.as_inspect_model."
            )
...
def as_inspect_model(
    ...
    *,
    ...
    the string registry, and renders as `aisteer/<model_name>`.

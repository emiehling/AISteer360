...
from aisteer360.spipe.codec import DecodeContext, decode, digest_of
from aisteer360.spipe.errors import SpipeFormatError, SpipeIntegrityError, SpipeSaveError, SpipeStaleError
from aisteer360.spipe.format import (
    ARTIFACTS_DIR,
    MANIFEST_NAME,
    pack_zip,
    read_manifest,
...
class SPipe:
    ...
    def recipe_id(self) -> str:
        ...
        from aisteer360.algorithms.core.identity import config_digest
    ...
    def _descriptor(self) -> dict:
        ...
        from aisteer360.algorithms.core.identity import config_descriptor_from_controls
    ...
    def verify(self) -> SpipeReport:
        ...
        from aisteer360.spipe.freeze import _toolkit_version

        saved = self._manifest.get("toolkit_version", "unknown")
        current = _toolkit_version()
        if saved.split(".")[0] != current.split(".")[0]:
            report_warnings.append(
                f"spipe was written by aisteer360 {saved}; this is {current} (major mismatch)."
            )
    ...
    def _instantiate(method_key: str, encoded_args: Mapping, ctx: DecodeContext):
        from aisteer360.algorithms.core.registry import RegistryError, resolve_method_key
    ...
    def load(
        ...
        *,
        ...
        Args:
            ...
            allow_code: Permit `$ref` imports, non-`aisteer360.` `$dc` imports, and
    ...
    def pipeline(
        ...
        **pipeline_kwargs,
        ...
        Args:
            ...
            **pipeline_kwargs: Forwarded to the `SteeringPipeline` constructor.

...
from aisteer360.spipe.errors import SpipeCodeRefError, SpipeFormatError, SpipeSaveError
from aisteer360.spipe.store import ArtifactRecord, ArtifactStore, tensors_payload

logger = logging.getLogger(__name__)

TAGS = ("$artifact", "$dc", "$component", "$ref", "$data", "$map")

TRUSTED_DC_PREFIX = "aisteer360."


def _trusted_dc_qualnames() -> frozenset[str]:
    """Qualnames of non-`aisteer360.` classes decodable without `allow_code`."""
...
class DataRef:
    ...
    def load(self):
        ...
        if self.kind == "path":
            ...
            from aisteer360.spipe.errors import SpipeIntegrityError
...
class DecodeContext:
    ...
    Attributes:
        ...
        allow_code: Whether `$ref` imports, non-`aisteer360.` `$dc` imports, and pickle-backed
...
def _encode_tree_object(value, save_fn, type_name: str, ctx: EncodeContext, path: str,
    ...
    from aisteer360.spipe.store import save_object_tree
...
def _encode_artifact_object(value: Any, ctx: EncodeContext, path: str, as_path: bool = False) -> dict | None:
    ...
    from aisteer360.algorithms.state_control.common.sources import VerifiedPrecomputed, _Precomputed
...
def _import_qualname(target: str, ctx: DecodeContext, path: str) -> type:
    ...
    if not trusted and not ctx.allow_code:
        raise SpipeCodeRefError(
            f"{path}: decoding {target!r} requires importing code outside the aisteer360 "
...
def _encode_component(value: Any, ctx: EncodeContext, path: str) -> dict | None:
    ...
    from aisteer360.algorithms.output_control.routed_decoding.routing import Predicate, Route, Router
...
def _encode_predicate(predicate, ctx: EncodeContext, path: str) -> dict:
    from aisteer360.algorithms.output_control.routed_decoding.routing import _And, _Decision, _Not, _Or
...
def _decode_component(value: Mapping, ctx: DecodeContext, path: str) -> Any:
    ...
    if kind == "router":
        from aisteer360.algorithms.output_control.routed_decoding.routing import Router
    ...
    if kind == "route":
        from aisteer360.algorithms.output_control.routed_decoding.routing import Route
    ...
    if kind == "decision":
        from aisteer360.algorithms.output_control.routed_decoding.routing import P
